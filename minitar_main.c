...
//         result = create_archive(archive, &files);
//     } else if (strcmp(operation, "-a") == 0) {    // Adds new files to a archive.
//         result = append_files_to_archive(archive, &files);
//     } else if (strcmp(operation, "-t") == 0) {    // Lists files in archive.
//         result = get_archive_file_list(archive, &files);
//     } else if (strcmp(operation, "-u") ==
//                0) {    // Replaces the old files of an archive with new ones.
//         result = append_files_to_archive(archive, &files);
//     } else if (strcmp(operation, "-x") ==
//                0) {    // Moves all files in a given archive and moves it to the current directory.
//         result = extract_files_from_archive(archive);
